...
import beeper_triage.runtime as rt
...
def test_resolve_base_url_uses_reachable_configured_url(monkeypatch):
    ...
    monkeypatch.setattr(rt.socket, "socket", lambda *a, **k: _fake_socket_ok())
    assert rt._resolve_base_url(agent=False) == "http://172.28.96.1:23399"


def test_resolve_base_url_falls_back_to_proxy_when_unreachable(monkeypatch):
    monkeypatch.setenv("BEEPER_BASE_URL", "http://172.28.96.1:23399")
    monkeypatch.setattr(rt.socket, "socket", lambda *a, **k: _fake_socket_refused())
    monkeypatch.setattr(rt, "_ensure_proxy", lambda: "http://127.0.0.1:23399")
    assert rt._resolve_base_url(agent=True) == "http://127.0.0.1:23399"


def test_resolve_base_url_uses_proxy_when_env_missing(monkeypatch):
    monkeypatch.delenv("BEEPER_BASE_URL", raising=False)
    monkeypatch.setattr(rt, "_ensure_proxy", lambda: "http://127.0.0.1:23399")
    assert rt._resolve_base_url(agent=False) == "http://127.0.0.1:23399"


def test_build_client_passes_resolved_url(monkeypatch):
    monkeypatch.setattr(rt, "_resolve_base_url", lambda *, agent: "http://h:1")
    captured = {}
    ...
    monkeypatch.setattr(rt, "BeeperClient", fake_client)
    rt._build_client("tok123", agent=False)
...
def test_resolve_base_url_warns_when_not_agent(monkeypatch):
    ...
    monkeypatch.setattr(rt, "_ensure_proxy", lambda: "http://127.0.0.1:23399")
    calls = []
    monkeypatch.setattr(rt.typer, "echo", lambda msg: calls.append(msg))
    rt._resolve_base_url(agent=False)
...
def test_resolve_base_url_silent_when_agent(monkeypatch):
    ...
    calls = []
    monkeypatch.setattr(rt.typer, "echo", lambda msg: calls.append(msg))
    rt._resolve_base_url(agent=True)
    assert calls == []
